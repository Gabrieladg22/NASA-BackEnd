...
public class JdbcNeoDao implements NeoDao{
    ...
    public Neo saveNeo(Neo neo, int userId, LocalDate date) {
        String sql = "INSERT INTO NEO(neo_id, user_id, neo_date, neo_name, hazardous, " +
                "min_diameter, max_diameter) VALUES " +
                "(?, ?, ?, ?, ?, ?, ?) RETURNING db_id";
        Integer dbId = jdbcTemplate.queryForObject(sql, int.class, neo.getId(), userId, date, neo.getName(),
                neo.isPotentiallyHazardousAsteroid(), neo.getEstimatedDiameter().getMiles().getEstimatedDiameterMin(),
